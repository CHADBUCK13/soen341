...
from ecommerce.api.account_context import (
    get_account_from_refresh_token,
    get_account_info,
    refresh_id_token,
)
from ecommerce.controllers.forms.itemForm import ItemForm
        ...
        if item_form.is_valid():
            ...
            if hasPaymentInfo(current_user["users"][0]["email"]):
                ...
                image_url = store_image(
                    request.FILES.get("image", None), item_form.data["name"]
                )

                item = Item(
                    add_item_form_data=item_form.data,
...
def search_items(request):
    """
    Searches items based on posted text from html
    """
